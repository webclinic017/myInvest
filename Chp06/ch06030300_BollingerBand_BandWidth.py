# ch06030300_BollingerBand_BandWidth.py
import sys
...
df['bandwidth'] = ((df['upper'] - df['lower']) / df['MA20']) *100  # Bollinger Band의 밴드폭을 나타내는 지표
df = df[19:]    # 맨 처음의 19개 요소는 20 이평값을 갖고 있지 않으므로, 무시하고, 20번째 요소를 첫번째 요소로 재지정한다.

print("\n---------- df ----------")
print(df, '\n')
df.to_excel('.\\chp06\\ch06030300_BollingerBand_BandWidth.xlsx')
